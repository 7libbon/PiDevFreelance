...
public function fetchOfferDetails(Request $request): JsonResponse
    ...
    public function search(Request $request, PaginatorInterface $paginator, LoggerInterface $logger): Response
    {
        ...
        $queryBuilder
        ->select('offre.id', 'offre.nom', 'offre.description','offre.echances','offre.statut','offre.prix','offre.idUser',) // Add other fields as needed
        ->from(Offre::class, 'offre')
        ->where('offre.nom LIKE :query')
        ->setParameter('query', '%' . $query . '%');
    
        // Log the SQL query
        $sqlQuery = $queryBuilder->getQuery()->getSQL();
        ...
        // Render the show.html.twig template with pagination data and search results
        return $this->render('offre/show.html.twig', [
            'pagination' => $pagination,
        ]);
    }
